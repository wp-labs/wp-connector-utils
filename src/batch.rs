...
/// field. Returns the original `data` unchanged when no injection is needed
/// (avoids clone overhead).
///
...
mod tests {
    ...
    fn inject_skips_when_oml_name_none() {
        ...
        let result = inject_oml_name(&meta, data);
        assert_eq!(result.len(), 1);
        // Should return the same Arc (no clone overhead)
        assert_eq!(Arc::as_ptr(&result[0]), original_ptr);
        // No wp_oml_name field injected
